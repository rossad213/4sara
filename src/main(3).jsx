...
function App() {
  ...
  const stats = useMemo(() => {
    ...
    const baseStats = { last, averageCycle, averagePeriod, nextPeriod, predictedEnd, ovulationDay, fertileStart, fertileEnd, reminderDate, daysUntil, currentCycleDay, minCycle, maxCycle, dataConfidence, confidenceNote, symptomStats, phaseInsights, totalEntries: periodEntries.length };
  ...
  const calendarData = useMemo(() => {
    ...
    const lastPeriodEnd = stats.last ? (stats.last.endDate || addDays(stats.last.startDate, stats.averagePeriod - 1)) : "";
    const follicularStart = lastPeriodEnd ? addDays(lastPeriodEnd, 1) : "";
    const follicularEnd = stats.fertileStart ? addDays(stats.fertileStart, -1) : "";
    const lutealStart = stats.fertileEnd ? addDays(stats.fertileEnd, 1) : "";
    const lutealEnd = stats.nextPeriod ? addDays(stats.nextPeriod, -1) : "";
    ...
    return Array.from({ length: cells }, (_, index) => {
      ...
      const isOvulation = key === stats.ovulationDay;
      const isFollicular = !entry && !isPredicted && !isFertile && !isOvulation && inRange(key, follicularStart, follicularEnd);
      const isLuteal = !entry && !isPredicted && !isFertile && !isOvulation && inRange(key, lutealStart, lutealEnd);
      const phaseLabel = entry || isPredicted ? "Menstruation" : isOvulation ? "Ovulation" : isFertile ? "Fertile" : isFollicular ? "Follicular" : isLuteal ? "Luteal" : "";
      ...
      <div className="container">
            ...
            <div className="brand-row">
              <button className="brand-home-btn" onClick={() => updateSettings({ welcomeSeen: false })} aria-label="Return to welcome screen">
                <span className="brand-mini-logo" aria-hidden="true">
                  <span className="mini-petal mini-left" />
                  <span className="mini-petal mini-center" />
                  <span className="mini-petal mini-right" />
                  <span className="mini-drop" />
                </span>
                <span>4Sara</span>
              </button>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 16, filter: "blur(4px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -10, filter: "blur(4px)" }} transition={{ duration: 0.25, ease: "easeOut" }}>
            ...
            {activeTab === "log" && <LogTab form={form} setForm={setForm} toggleSymptom={toggleSymptom} saveEntry={saveEntry} editingId={editingId} cancelEdit={() => { setEditingId(null); setForm(blankForm()); }} entries={sortedEntries} startEdit={startEdit} deleteEntry={deleteEntry} allSymptoms={allSymptoms} customSymptoms={settings.customSymptoms || []} customSymptomInput={customSymptomInput} setCustomSymptomInput={setCustomSymptomInput} addCustomSymptom={addCustomSymptom} removeCustomSymptom={removeCustomSymptom} />}
    ...
    <div className="welcome-screen">
      <div className="welcome-shell">
        <div className="welcome-logo" aria-hidden="true">
          <div className="lotus-mark large">
  ...
  return (
    <main>
      <Card className="pad">
        ...
        <p className="calendar-note">Cycle phases and fertile timing are estimates and should not be used as birth control or medical advice.</p>
