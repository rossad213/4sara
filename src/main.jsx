...
const WELCOME_SCREEN_LOGO = "/icons/icon-512.png";
...
function isPublicRoutePath(path = currentPath()) {
  return Boolean(publicTabFromPath(path));
}
...
function buildCyclePatternNotes(entries, stats) {
  const notes = [];
  const safeEntries = Array.isArray(entries) ? entries : [];
  const recentEntries = [...safeEntries]
    .filter((entry) => entry?.startDate && !isFutureDate(entry.startDate))
    .sort((a, b) => new Date(b.startDate) - new Date(a.startDate))
    .slice(0, 12);

  const recentSymptoms = Object.entries(recentEntries.flatMap((entry) => entry.symptoms || []).reduce((acc, symptom) => {
    acc[symptom] = (acc[symptom] || 0) + 1;
    return acc;
  }, {})).sort((a, b) => b[1] - a[1]);

  const recentMoods = Object.entries(recentEntries.flatMap((entry) => normalizeMoods(entry).filter((mood) => mood !== "N/A")).reduce((acc, mood) => {
    acc[mood] = (acc[mood] || 0) + 1;
    return acc;
  }, {})).sort((a, b) => b[1] - a[1]);

  const noteText = recentEntries.map((entry) => entry.notes || "").join(" ").toLowerCase();
  const noteSignals = [
    ["stress", ["stress", "stressed", "anxiety", "overwhelmed"]],
    ["sleep changes", ["sleep", "insomnia", "tired", "exhausted", "restless"]],
    ["travel", ["travel", "trip", "flight", "vacation", "timezone"]],
    ["illness", ["illness", "sick", "fever", "cold", "flu"]],
    ["medication", ["medication", "medicine", "antibiotic", "prescription"]],
    ["exercise changes", ["exercise", "workout", "training", "running", "gym"]],
    ["diet or appetite changes", ["diet", "appetite", "eating", "craving", "meal"]]
  ].filter(([, terms]) => terms.some((term) => noteText.includes(term))).map(([label]) => label);

  if (!safeEntries.length) {
    return [{
      title: "Start with a few logs",
      body: "Pattern notes will appear after 4Sara has menstruation entries, daily check-ins, moods, symptoms, or notes to compare.",
      details: ["Add check-ins when something feels different.", "Use notes for stress, sleep, travel, illness, medication, or routine changes."],
      tone: "rose"
    }];
  }

  if (stats.currentCycleAdjustmentDays > 0 && stats.currentCycleFactors?.length) {
    notes.push({
      title: "Your timing estimate moved slightly",
      body: `4Sara noticed repeated current-cycle context: ${readableList(stats.currentCycleFactors)}. The prediction was only nudged gently, not heavily changed.`,
      details: [`Current capped timing nudge: +${stats.currentCycleAdjustmentDays} day${stats.currentCycleAdjustmentDays === 1 ? "" : "s"}.`, "This is an observation from logs, not a medical conclusion."],
      tone: "amber"
    });
  }

  if (stats.minCycle && stats.maxCycle && stats.maxCycle - stats.minCycle >= 7) {
    notes.push({
      title: "Cycle length has been changing",
      body: `Logged cycle lengths range from ${stats.minCycle} to ${stats.maxCycle} days. That wider range can make predictions less exact.`,
      details: ["Daily check-ins may help show whether changes line up with stress, sleep, travel, illness, medication, or routine shifts.", "Prediction dates should be treated as estimates."],
      tone: "amber"
    });
  }

  if (stats.outlierCycles?.length) {
    notes.push({
      title: "An unusual cycle was softened",
      body: `4Sara found ${stats.outlierCycles.length} cycle${stats.outlierCycles.length === 1 ? "" : "s"} that looked outside the recent typical range, so it reduced the effect on future predictions.`,
      details: ["This helps one unusual delay avoid permanently changing the average.", "Keep logging to confirm whether it was a one-off or a new pattern."],
      tone: "purple"
    });
  }

  if (noteSignals.length) {
    notes.push({
      title: "Notes mention possible context",
      body: `Recent notes mention ${readableList(noteSignals.slice(0, 4))}. These may be worth tracking next to timing, moods, and symptoms.`,
      details: ["4Sara does not assume these caused a change.", "It simply surfaces them so they are easier to notice."],
      tone: "sky"
    });
  }

  if (recentSymptoms.length || recentMoods.length) {
    const symptomText = recentSymptoms.slice(0, 3).map(([symptom, count]) => `${symptom}${count > 1 ? ` (${count})` : ""}`);
    const moodText = recentMoods.slice(0, 3).map(([mood, count]) => `${mood}${count > 1 ? ` (${count})` : ""}`);
    notes.push({
      title: "Recent check-in pattern",
      body: [
        symptomText.length ? `Symptoms showing up: ${readableList(symptomText)}.` : "",
        moodText.length ? `Moods showing up: ${readableList(moodText)}.` : ""
      ].filter(Boolean).join(" "),
      details: ["If these repeat around the same phase, they may become a stronger pattern in Insights.", "Use daily check-ins to make this more accurate."],
      tone: "rose"
    });
  }
  ...
  if (!notes.length) {
    notes.push({
      title: "No major change pattern yet",
      body: "Nothing strong is standing out from recent entries yet. That is normal when there are only a few logs or when cycles are steady.",
      details: ["Keep logging symptoms, moods, and notes when something feels different.", "Pattern notes become more useful as history builds."],
      tone: "green"
    });
  }
...
function calculateStatsForEntries(sourceEntries, sourceSettings) {
  ...
  return {
    ...baseStats,
    dynamicSuggestions: buildSuggestions(baseStats),
    cyclePatternNotes: buildCyclePatternNotes(safeEntries, baseStats)
  };
}
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        setActiveTabRouteUnlessPublic("dashboard");
      }
      setSupportViewers({});
      setSupportActivity([]);
  ...
  const setActiveTabRouteUnlessPublic = (tab, { replace = false } = {}) => {
    setActiveTab(tab);
  ...
  useEffect(() => {
    const syncRouteToState = () => {
      ...
      if (nextPublicTab) {
        // Public routes are the source of truth. If the user edits the URL back to "/",
        // stay on the public home page instead of reopening the last app tab.
  ...
  const signedInAccountStillLoading = Boolean(authUser && !currentPublicTab && (!accountDataChecked || !cloudCheckedForAccount || cloudLoadedAccountUid !== authUser.uid));
      ...
      <Card className="pad main-col full-width-card account-polished-card">
        ...
        {authLoading ? (
          ...
          <div className="auth-panel account-auth-polished">
            ...
            <div className="no-account-local-card">
              <h3>Using 4Sara without an account?</h3>
              <p>Your data is saved only on this device/browser. It is not backed up to cloud unless you create an account or log in and save it.</p>
              {confirmClearLocal && (
                <p className="danger-confirm">Confirm clear: click Clear local data again to permanently remove the tracker data saved on this device.</p>
              )}
              <div className="account-actions">
                <Button onClick={clearData} variant="secondary">
                  {confirmClearLocal ? "Confirm clear local data" : "Clear local data"}
                </Button>
  ...
  const SupportPage = () => (
    <section className="landing-info-page">
      ...
      <div className="support-tier-grid landing-support-grid">
        ...
        <div className="support-tier-card custom-support-card"><span className="support-tier-label">Custom support</span><h3>One-time payment</h3><p>Make a one-time payment through Stripe.</p><Button onClick={() => openSupportLink(supportLinks.stripeCustom)} variant="secondary">Choose one-time payment</Button></div>
      </div>
      <div className="support-note-box"><strong>Secure checkout</strong><p>Stripe securely handles the support checkout. Monthly options are recurring, and the one-time payment option lets supporters make a single contribution.</p></div>
    </section>
  );
    ...
    <div className="welcome-screen">
      <div className="welcome-shell account-prompt-shell">
        <div className="welcome-logo" aria-hidden="true">
          <img src={WELCOME_SCREEN_LOGO} alt="" />
        </div>
...
function pickDashboardPredictionText(options, seed = "") {
  if (!options?.length) return "";
  const source = String(seed || todayKey());
  let total = 0;
  ...
  return options[Math.abs(total) % options.length];
}

function dashboardPredictionTextGroup(stats, hasLoggedData) {
  if (!hasLoggedData) {
    return [
      "4Sara is learning your rhythm from the first pieces of your cycle history.",
      "Your pattern is just starting to take shape.",
      "A first estimate is forming from what you’ve logged so far."
    ];
  }

  if (stats.currentCycleAdjustmentDays) {
    return [
      "Recent check-ins suggest this cycle may be moving a little differently.",
      "Your notes may be hinting at why this cycle feels slightly shifted.",
      "4Sara noticed recent context that may gently shift this cycle’s estimate."
    ];
  }

  if (stats.outlierCycles?.length) {
    return [
      "One unusual cycle was noticed, but it won’t take over your whole pattern.",
      "4Sara softened an unusual cycle so your average stays more balanced.",
      "A recent cycle looked different, so 4Sara is treating the estimate with extra care."
    ];
  }

  if (stats.completedPeriodEpisodes >= 3) {
    return [
      "Your logged cycle pattern is giving 4Sara a clearer prediction window.",
      "Your cycle history is starting to tell a steadier story.",
      "Your recent logs are helping 4Sara narrow the estimate."
    ];
  }

  if (stats.totalEntries >= 1) {
    return [
      "Your cycle rhythm is becoming clearer with each logged period.",
      "4Sara is reading your recent pattern, not just guessing from a default cycle.",
      "Your next period is estimated from the rhythm your logs are building."
    ];
  }

  return [
    "Your next period is estimated from your logged cycle history."
  ];
}
...
function Dashboard({ stats, settings, sortedEntries, startEdit, deleteEntry, jumpToNextPeriod, setActiveTab, setLocked }) {
  ...
  const dashboardPredictionNote = pickDashboardPredictionText(
    dashboardPredictionTextGroup(stats, hasLoggedData),
    `${stats.nextPeriod || todayKey()}-${stats.totalEntries || 0}-${stats.currentCycleAdjustmentDays || 0}-${stats.outlierCycles?.length || 0}-${stats.completedPeriodEpisodes || 0}`
  );
      ...
      <section className="main-col dashboard-polished-main">
        <Card className="dashboard-soft-hero-card">
          <div className="dashboard-soft-hero-copy">
            ...
            <p className="dashboard-hero-note">{dashboardPredictionNote}</p>
            {hasLoggedData && <p className="prediction-education-note">Predictions improve as you log more cycles. Early estimates may change.</p>}
...
function Insights({ stats, settings, setLocked, isSupportView = false }) {
  ...
  const profileDisplayName = isSupportView ? settings?.profileName?.trim() : "";
      ...
      <Card className="pad main-col insights-main-card">
        <div className="insights-hero">
          <div>
            ...
            <h2>{profileDisplayName ? `${profileDisplayName} at a glance` : "Your cycle at a glance"}</h2>
        ...
        <div className="insights-section-card pattern-notes-card">
          <div className="insights-section-head">
            <h3>Why might this be changing?</h3>
            <p className="muted">Cycle Pattern Notes look at recent timing, notes, moods, and symptoms. They are observations, not medical conclusions.</p>
          </div>

          <div className="pattern-note-grid">
            {(stats.cyclePatternNotes || []).map((note, index) => (
              <div key={`${note.title}-${index}`} className={`pattern-note-card ${note.tone || "rose"}`}>
